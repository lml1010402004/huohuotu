package com.yitoa.viewpagerdemo;
...
public class CycleViewPager extends Fragment implements OnPageChangeListener {
	...
	public void setData(Context context,List<ImageView> views, List<ADInfo> list, ImageCycleViewListener listener) {
		setData(context,views, list, listener, 0);
	}
	 ...
	 *            默认显示位置
	 */
	public void setData(Context context,List<ImageView> views, List<ADInfo> list, ImageCycleViewListener listener, int showPosition) {
	...
	private void setIndicator(int selectedPosition) {
		for (int i = 0; i < indicators.length; i++) {
			indicators[i].setBackgroundResource(R.drawable.pot2);
		}
		if (indicators.length > selectedPosition)
			indicators[selectedPosition]
					.setBackgroundResource(R.drawable.pot);
	}

	/**
	 * 如果当前页面嵌套在另�?个viewPager中，为了在进行滚动时阻断父ViewPager滚动，可�? 阻止父ViewPager滑动事件
